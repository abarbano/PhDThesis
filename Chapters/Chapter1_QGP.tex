...
and between $\Lambda$ and $\bar{\Lambda}$ is due to their quark content, 
$K^+ (u\bar{s})$, $K^- (\bar{u}s)$, $\Lambda (uds)$, $\bar{\Lambda} (\bar
{u}\bar{d}\bar{s})$. Indeed, at low energies, most of the produced hadrons will 
reflect the quark composition of the colliding nucleons. When the production of
 the $\bar{u}, \bar{d}$ quarks becomes accessible with increasing energy, the 
 symmetry is restored also for these particles.
\begin{figure}[!t]
  \centering
...
For such reason, Lattice Calculation are an indispensable technique. 
...
Lattice QCD gives a plasma temperature of about 210 MeV, that corresponds 
to $\sim 10^{10}$ K (to give an idea, the temperature inside the Sun is 
around 11$\times 10^{6}$ K). \\Let's now briefly go through the main discoveries 
that all together formed pieces of evidence to state the observation of a new state of matter.
...
the magnitude of strangeness production is usually estimated by measuring 
...
Quark-Gluon Plasma. The binding energy of the $c\bar{c}$ pair is of 
the order of few hundreds MeV, but indeed small if compared to the mean 
temperatures inside the QGP. Thus, the expectation is that such a pair 
inside the medium could undergo dissociation, due to the presence of free 
color charges in the QGP that cause Debye screening of the colour charge of 
the two quarks~\cite{Abreu:2000ni}. Quarkonia states with different bounding 
energy are also expected to dissociate at different temperatures~\cite{Digal:2001ue}.
...
as a function of the energy density of the medium traversed by the charmonium state. 
...
invariant mass distribution of $\mu^+\mu^-$ ($e^+ e^-$) from $\rho$ meson decays. 
The $\rho$ meson was used from the very beginning as the test particle for in-medium 
...
We will go now through a revision of some of the most important results and 
open points regarding heavy-ion physics at the higher LHC energies. We will 
start with the characterization of the centrality of the collision, necessary to 
correctly interpret the properties of the so formed medium, to go then deeper 
inside the properties of the QGP.
...
Under the assumptions that, (i) at sufficiently high energies, the nucleons 
...
while the equilibrium model predicts them very nicely. Other models 
...
\end{equation}
It has been experimentally verified in pp collisions the universality 
...
and the velocity is related to the radial flow, i.e. the collective velocity
 arising from the expansion of the fireball. The first term still accounts
  for the Brownian motion of the particles, present in Eq.~\ref{eq:SpectraBZ}.
   ...
   velocity distribution $\beta_r(r)$ in the region \mbox{$0 \leq r \leq R$} 
...
%   \includegraphics[height=8cm]{FigCap1/BWfit.pdf}
 ...
 \caption{Left: $\pi$ spectra in different centrality classes in Pb-Pb collisions at $\sNN = $ 5.02 TeV~\cite{Jacazio:2017dvy}. Right: $T_{kin}$ vs $\langle \beta_T \rangle$ from blast-wave fits for different colliding systems and energies~\cite{Jacazio:2017dvy}.}
...
The blast-wave fit to the measured spectra (an example for pion
 production in Pb-Pb collisions at $\sNN = $ 5.02 TeV is shown 
 in Fig.~\ref{fig:BWfit_BetaTplot} left~\cite{Jacazio:2017dvy}) allows
  an estimate of the kinetic freeze-out temperature $T_{fo}$ and 
  surface velocity $\beta_s$.
Fig.~\ref{fig:BWfit_BetaTplot} (right) shows the ALICE results 
for the blast-wave parameters ($T_{kin}$ and $\langle \beta_{\perp}\rangle$ in the figure)
 for different colliding systems and energies. Blast-Wave parameters
...
% With the large amount of data provided by the LHC, it became accessible
%  to study not only the on-average anisotropies but the event-by-event flow 
 % fluctuations~\cite{Aad:2013xma,Schukraft:2012ah}, to which higher Fourier 
 ...
 Fig.~\ref{fig:vnHydro} shows
   ...
   Fourier coefficients $v_n$ up to 5th order measured by ATLAS~\cite{ATLAS:2012at}
    ...
    (right panel) in Au-Au collisions at $\sNN = 200 $ GeV. The value
     of the shear viscosity $\eta/s$ of the medium from the hydro fits 
     in Au-Au collisions at top of RICH energy is 0.12, while the value
...
particle species, that can further constrain the models. In Fig.~\ref{fig:PiKPRaa5TeV}
 ...
 A second observation from 
  ...
  low $\pt$ that is a direct consequence of the radial flow. At high $\pt$
...
The interaction of the high-$\pt$ partons with the color field of the medium
 ...
 energy of leading parton) and collinear ($k_{\perp} \ll \omega$, being $k_{\perp}$
