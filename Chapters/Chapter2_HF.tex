...
$\sim 0.02$ fm for beauty. Maximum
reachable time scales are instead around 1 fm/c (0.1 fm/c) for charm 
(beauty) quarks with energy of 15 GeV (about 1 fm/c for beauty 
quark jets with energy of about 150 GeV).
...
unless the initial QGP temperatures occur to be much larger than 
...
In contrast with FONLL and GM-VFNS, that are based on NLO 
perturbative QCD calculations and are limited to inclusive production
...
final states of high-energy collisions in full detail and contain a large 
...
The first generator that did the effort of matching NLO calculations 
...
The NLO calculations for the hard processes are obtained within the 
...
characterize a class of phenomena that are expected to break the binary scaling in 
...
their origin is mainly related to modification of the PDFs for nucleons bound in nuclei,
...
can affect the partons in the nucleons (initial-state effects) as well as
...
formation of a QGP droplet in small collision systems.\\
...
Fig.~\ref{fig:RpA} (left panel)~\cite{ALICEPAS2017008}. Data are compared to 
theoretical models that include CNM effects. In general, the 
calculations describe the data within 
uncertainties in the entire $\pt$ range. CNM effects 
are expected to be largest at low $\pt$, 
where, in addition, the predictions of the different 
theoretical approaches differ and the statistical 
uncertainty of the present measurement for the 
...
$\pt$ measured in Pb-Pb collisions at $\sNN = 5.02 $ TeV are shown in Fig.~\ref{fig:Raa} (left), 
as a preliminary results for the 0--10\%, 30--50\% and 60--80\% centrality classes~\cite{ALICE-PUBLIC-2017-003}. 
...
\begin{figure}[!ht]
  ...
  \caption{Left: Comparison of the D-meson~\cite{Adam:2015nna} and charged-pion~\cite{Abelev:2014laa} $\RAA$ as a function of centrality expressed in terms of $\langle N_{\rm part} \rangle $ in 8 $< \pt < 16$ $\Gevc$~\cite{Adam:2015nna} 
and of the $\RAA$ of non-prompt J$/\psi$ mesons in 6.5 $< \pt < 30$ $\Gevc$ measured by CMS~\cite{Khachatryan:2016ypw}. 
Right: $\RAA$ of electrons from beauty-hadron decays as a function of $\pt$ together with
...
is found to be compatible to lattice QCD calculations.
