...
They are due to interactions between the EM fields of relativistic heavy ions, that can lead
to photo-production processes, photo-nuclear interactions and electromagnetic dissociation (EMD).
Photo-production processes generally result in the creation of an $e^+e^-$ pair.
In the case of a single photo-production event, the event multiplicity is asymmetric within the event.
...
In particular, among photo-nuclear processes, electromagnetic dissociation in which the sudden EM pulse
...
and its associated covariance matrix are determined. The resolution of the distance of closest approach to the 
primary vertex in the transverse plane for charged-particle ITS-TPC tracks in pp, p-Pb and Pb-Pb collisions is shown in Fig.~\ref{fig:DCAxyResolAndPtResol} (left). 
...
found with SPD and with global tracks are shown in the left panel of Fig.~\ref{fig:VtxResol}. 
...
\end{equation}
where N is the number of decay particles (3 in case of $\DstoKKpi$ dacys) and $d_i$ is the distance between the track $i$ and the vertex 
...
(right) as example for the $\DplustoKpi$ meson. The 
resolution improves with increasing $\Dplus$ meson $\pt$ and is better in the plane prependicular to the $\Dplus$ $\pt$ direction.
...
A participant nucleon of one nucleus is defined as a nucleon that undergoes one or more 
 ...
 model, the N-N inelastic cross section and the nucleon density profiles of the projectile and target
...
For a standard running year, an order of $10^9$ pp events and $10^8$ Pb-Pb events are collected, for a total 
