...
\item \textbf{Single-track normalised impact parameter residual $d_0$} : it is defined 
...
In Fig.~\ref{fig:var3} (right) the distributions of the $D_{d_{0}} = max[d^{reco}_{0}-d^{exp}_{0}/\sigma]$ among 
...
feed-down components. Since the distributions of $D_{d_{0}}$ 
...
that of prompt D mesons. In this analysis selections on the $D_{d_{0}} \sim 2$
were applied.
\end{itemize}
...
pion mass to the two same-sign tracks. In fact, considering the $\Dsplus$
 decay, the charge configuration of the tracks (+, -, +) can be interpreted
  both as to ($K^+,K^-,\pi^+$) and ($\pi^+,K^-,K^+$) mass assignements. Signal candidates with 
  ...
  a contribution to the invariant mass distributions that could introduce
   a bias in the extraction of the raw yield of $\Dsplus$ mesons. It was verified, both in 
...
The rapidity acceptance correction factor $C_{\Delta y}=2\,y_{\rm fid}(\pt)$ assumes
...
of $\pt$ and are shown in Fig.~\ref{fig:fprompt}.
\begin{figure}[!t]
\begin{center}
...
The RMS of the distributions of the multiple trials an be used as an estimator of
...
the systematic uncertaintiy on the yield extraction via the fit procedure 
...
respect to MC value,  (iv) free peak position, (v) peak position fixed to MC value.
The uncertainty was estimated as the maximum error (max. value - min. value) divided by $\sqrt{12}$.
It resulted to be $\sim$5\% in all the $\pt$ intervals. It is reduced 
by a factor 3-4 with respect to the analysis of the old reconstrunction,
...
possible imperfections in the description in the simulation of the variables used to select the signal, which could introduce a bias in the efficiency. A possible test to quantify the effect of imperfections in the 
...
a factor from 2 to 6, depending on the $\pt$ interval, between the loosest and  
the tightest sets of cuts considered. We took the RMS 
of the corrected yield distributions in each $\pt$ interval as an estimator of 
...
that the statistical significance is $<3$ and the S/B os lower than ??.
of the signal peak larger than 3. The RMS of the corrected yield distributions for the considered
...
Matching efficiency for primary tracks is higher than 
...
Finally, in Fig.~\ref{fig:Fractions}, the data-driven values of primary and secondary
 ...
 the ones from the PYTHIA + GEANT 3 simultion (empty markers). 
...
\end{center}
\end{figure}
Based on these ratios, a systematic uncertainty of 2\% per track was assigned for particles 
with $2< \pt <6$ $\Gevc$, and of 1\% at lower and higher $\pt$.
...
efficiency for 2- and 3-prong D-meson decays as a function of D-meson $\pt$, 
...
{\bf There is also an uncertainty due to the B-meson admixture, i.e. 
the fractions $f(b\rightarrow {\rm B}^0, {\rm B}^+, {\rm B}_s, \Lambda_b)$.}
...
comparison between LHCb B$^{+}$-meson cross-section and the FONLL
 calculations shows a good agreement. As regards $\Lambda_{\rm b}$ cross section 
(top right panel), FONLL calculations for B mesons describe the shape of data 
but need a rescaling of a factor 2 (which may be related to the 
large uncertainties on the determination of the BR of the channel $\Lambda_{b} \rightarrow J/\psi \,{\rm p\, K}$ 
...
\caption{Fraction of $\Ds$ originating from decays of different species of beauty hadrons.}
\label{fig:DsParents}
\end{center}
\end{figure}
Considering that:
...
in the measuremtent was also evaluated. 
...
in Table~\ref{tab:SystDs}. The $\Dsplus$-meson production 
