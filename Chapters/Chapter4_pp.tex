...
cuts described in this Section and were thus selected for the analysis. 
The larger data sample of the current reconstruction (the number of events was 300M for the previous
reconstruction) was obtained thanks to improvements in the reconstruction
and detector alignments. A minimum-bias (MB)
...
tracklets associated to it (N$^{\rm trkl}_{\rm min}$) and if the 
$z$-separation between primary and pile-up vertex ($\Delta z$) is larger than a certain value.
In this analysis, N$^{\rm trkl}_{\rm min}>3$ and $\Delta z>0.6$ cm were used.
...
The percentage of events rejected during the vertex and physics selection
is shown in Fig.~\ref{fig:vxtReso} (right panel). The most important contribution comes from the vertex selections.

\begin{figure}[!t]
\centering
\includegraphics[width=.5\textwidth]{FigCap4/VtxReso.png}
\includegraphics[width=.48\textwidth]{FigCap4/RejFracPass4.png}
...
The $\Dspm$ candidates are built from combinatorial 
 ...
 signs. In this way, a large number of candidates is created, most of them being combinatorial background. 
 ...
 from background candidates. 
To give an idea, there are about 885M $\Ds$ candidates in the 370M analysed events, after a 
preliminary filtering selections with very loose cuts. Candidates 
...
\item \textbf{Normalised decay length NDL$_{\rm xy}$}, defined as the decay length 
...
\section{Invariant mass spectra, cut optimisation and signal extraction}
...
The integrated luminosity $L_{\rm int} = (6.0 \pm 0.2)\, {\rm nb}^{-1}$ was computed as $L_{\rm int} = N_{\rm ev}/\sigma_{\rm pp,MB}$,
where $N_{\rm ev}$ is the number of analysed events and 
$\sigma_{\rm pp,MB} = 62.2$ mb~\cite{Abelev:2012sea}
...
The RMS of the distribution of the multiple trials on fit was used as an estimator of
...
The relative difference between the means of the yield distributions
...
Nevertheless, it is important to look also at the values of the 
...
\caption{Ratio of DCA$_{\rm xy}$ distributions in data and in the fit result histograms, in $\pt$ intervals from 0.5 to 15 $\Gevc$.}
...
\label{eq:fc}
\end{equation}
The use of this alternative approach has been reconsidered after a
...
for the specific centre-of-mass energy and rapidity interval.
